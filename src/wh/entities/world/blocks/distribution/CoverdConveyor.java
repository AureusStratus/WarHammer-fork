...
public class CoverdConveyor extends Block implements Autotiler{
    ...
    public TextureRegion[] coverdRegions;
    ...
    public CoverdConveyor(String name){
        super(name);
        size = 1;
        rotate = true;
        update = true;
    ...
    public void handlePlacementLine(Seq<BuildPlan> plans){
        ...
        if(bridgeReplacement instanceof DuctBridge bridge) Placement.calculateBridges(plans, bridge, hasJuntionReplacement, b -> b instanceof Duct || b instanceof CoverdConveyor);
        if(bridgeReplacement instanceof ItemBridge bridge) Placement.calculateBridges(plans, bridge, hasJuntionReplacement, b -> b instanceof CoverdConveyor);
    }
    ...
    public class CoConveyorBuild extends Building implements ChainedBuilding {
        ...
        public @Nullable CoConveyorBuild nextc;
        ...
        public void overwrote(Seq<Building> builds){
            if(builds.first() instanceof CoConveyorBuild build){
                ids = build.ids.clone();
                xs = build.xs.clone();
                ys = build.ys.clone();
                len = build.len;
