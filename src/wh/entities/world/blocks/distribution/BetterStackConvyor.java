...
public class BetterStackConvyor extends StackConveyor{
    public TextureRegion edge2Region;
    public BetterStackConvyor(String name){
        super(name);
    }
    @Override
    public void load() {
      ...
      public void draw(){
          ...
          for (int i = 0; i < 4; i++) {
              if ((blendprox & (1 << i)) == 0) {
                  ...
                  if (rot <= 1) Draw.rect(edge2Region, x, y, rot * 90);
                  else Draw.rect(edgeRegion, x, y, rot * 90);
              }
          }
