 ...
 */
public class AccelLaserBulletType extends BulletType {
    ...
    public AccelLaserBulletType(float damage) {
        this.damage = damage;
        speed = 0f;
        ...
        hittable = false;
        absorbable = false;
        lightningColor = Color.valueOf("ec7458aa");
    }
    ...
    public void update(Bullet b) {
        super.update(b);
        if (b.timer(1, 5f) || Mathf.chanceDelta(0.004)) {
            PositionLightning.createEffect(b, Tmp.v2.trns(b.rotation(), b.fdata ).add(b), lightningColor, 1, Mathf.random(2, 3));
        }
                ...
                WHUtils.collideLineRawEnemyRatio(b.team, b.x, b.y, Tmp.v1.x, Tmp.v1.y, collisionWidth, (building, ratio, direct) -> {
                    ...
                    if (direct) {
                        if (h) {
                            // 如果有穿透限制，增加穿透得分
                            if (p)
                                data.pierceScore += building.block.size * (building.block.absorbLasers ? 3f : 1f) * ratio;
                            // 如果没有穿透限制或穿透得分超过限制
                    ...
                    if (h) {
                        // 如果有穿透限制，增加穿透得分
                        if (p)
                            data.pierceScore += (((unit.hitSize / Vars.tilesize) / 2f) + (unit.health / 4000f)) * ratio;
                        // 如果没有穿透限制或穿透得分超过限制
                        if (!p || data.pierceScore >= pierceCap) {
                            // 计算临时向量
                            Tmp.v2.trns(b.rotation(), maxLength * 1.5f).add(b);
    ...
    public static class LaserData {
        public float velocity, velocityTime, targetSize, pierceOffset, pierceOffsetSmooth, pierceScore, restartTime = 5f;
